...
# --- 在此处手动设置风险阈值 ---
# 您可以在这里直接修改数值来改变高低风险的判断标准。
# 例如: 0.43 表示当模型预测概率 >= 43% 时，判定为“高风险”。
risk_threshold = 0.88 
...
def load_model():
    try:
        # 加载 final_model.joblib 文件
        saved_object = joblib.load('final_model.joblib')
        # 兼容新旧两种保存格式
        if isinstance(saved_object, dict):
            return saved_object.get('model') # 如果是字典，则提取模型
        else:
            return saved_object # 如果直接是模型，则返回
...
with col1:
    age = st.slider('年龄 (岁)', 18, 100, 50, key="age_slider")
    height_cm = st.number_input('身高 (cm)', min_value=100.0, max_value=220.0, value=170.0, step=0.5, key="height_input")
    weight_kg = st.number_input('体重 (kg)', min_value=30.0, max_value=200.0, value=70.0, step=0.5, key="weight_input")

with col2:
    neck_circumference = st.slider('颈围 (cm)', 20, 60, 38, key="neck_slider")
    asa_grade = st.selectbox('ASA分级', [1, 2, 3, 4], index=0, key="asa_select")
    hypertension = st.selectbox('是否有高血压', ['无', '有'], index=0, key="hypertension_select")
    
with col3:
    snoring = st.selectbox('是否打鼾', ['否', '是'], index=0, key="snoring_select")
    # 自动计算BMI并显示
    if height_cm > 0:
